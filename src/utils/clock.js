...
const incrementDate = (date, { weeks = ZERO, days = ZERO, hours = ZERO, minutes = ZERO, seconds = ZERO }) => {
  return new Date(
    date.getTime() +
      ...
      days * TIME.A_DAY +
      hours * TIME.AN_HOUR +
      minutes * TIME.A_MINUTE +
      seconds * TIME.A_SECOND
  );
};
...
const clockBetweenDates = (
  date1,
  date2,
  options = { isWeeksRemoved: false, isDaysRemoved: false, isHoursRemoved: false, isMinutesRemoved: false }
) => {
  ...
  const hours = options.isHoursRemoved
    ? ZERO
    : Math.floor((diff - weeks * TIME.A_WEEK - days * TIME.A_DAY) / TIME.AN_HOUR);
  const minutes = options.isMinutesRemoved
    ? ZERO
    : Math.floor((diff - weeks * TIME.A_WEEK - days * TIME.A_DAY - hours * TIME.AN_HOUR) / TIME.A_MINUTE);
  const seconds = Math.floor(
    (diff - weeks * TIME.A_WEEK - days * TIME.A_DAY - hours * TIME.AN_HOUR - minutes * TIME.A_MINUTE) / TIME.A_SECOND
  );
...
const getFormattedClock = (date) => {
  ...
  const format12Hour =
    givenDate.getHours() > LARGEST_SINGLE_DIGIT ? givenDate.getHours() % MOD12 : givenDate.getHours();
...
const getFormattedDate = (date) => {
  ...
  const timeBetweenNowAndNewYears = clockBetweenDates(givenDate, newYears) ?? { weeks: 1, days: 1 };

  const displayDate =
    DAYS_OF_THE_WEEK[givenDate.getDay()] +
    ', ' +
    MONTHS[givenDate.getMonth()].slice(ZERO, NUMBER_OF_LETTERS) +
    ' ' +
    givenDate.getDate();
  const displayWeek = Math.trunc(
    firstWeekPartial + timeBetweenNowAndNewYears.weeks + timeBetweenNowAndNewYears.days / DAYS_IN_A_WEEK
  );
